...
const Sidebar = () => {
  ...
  return (
    <div className="w-64 h-screen bg-black shadow-lg fixed top-0 left-0 flex flex-col text-white">
      <div className="p-6 font-bold text-xl border-b border-gray-200">Employee Management</div>
      <nav className="flex-1 p-4 space-y-4">
        {authNavItems.map((item) => (
          <Link
            key={item.to}
            to={item.to}
            className={`flex items-center p-3 rounded-lg transition duration-300 ${location.pathname === item.to ? "text-black bg-[#F7F7F7] font-semibold" : ""
              }`}
          >
